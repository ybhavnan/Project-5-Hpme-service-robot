...
int main(int argc, char** argv){
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    ...
    ROS_INFO("The base failed to reach pickup location for some reason");

  while(!ac.waitForServer(ros::Duration(5.0))){
    ROS_INFO("Waiting for 5 seconds before heading to the drop-off location");
  }

  // Define a position and orientation for drop-off location 
